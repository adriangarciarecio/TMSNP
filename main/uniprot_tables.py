...
pfam_download_path = (
    "./pfam_download/"
)  # !!Cal posar el PATH correcte de la carpeta "TFM/pfam/pfam_download" que en aquest cas es: "/home/alumne/Escritorio/TFM/pfam/pfam_download"
pfam_download_raw = (
    "./pfam_download_raw/"
)  # !! CAl posar EL PATH DE LA CARPETA: "TFM/pfam/pfam_download_raw" QUE EN AQUEST CAS ES: "/home/alumne/Escritorio/TFM/pfam/pfam_download_raw"
...
llista_snps_transmembrana_taula_snip_regions = descart_regio_no_transmembrana(
...
for num_posicio_llista in range(len(data_taula_protines)):
    for num_pfam in range(len(data_taula_protines[num_posicio_llista][3])):
        ...
        mycursor.execute(
            ("INSERT INTO receptor_pfam VALUES" + str((accesion_protein, pfam_protein)))
        )
        conn.commit()

for num_posicio_llista in range(len(llista_snps_transmembrana_taula_snip_regions)):
    for numero_snips in range(
        len(llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][2])
    ):
        accesion_protein = llista_snps_transmembrana_taula_snip_regions[
            num_posicio_llista
        ][0]
        var_snip = llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][2][
            numero_snips
        ]
        change_snip = llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][
            3
        ][numero_snips]
        snip_inici_simbol = change_snip[: change_snip.find("->")]
        snip_final_simbol = change_snip[change_snip.find("->") + 3 :]
        rs_snip = llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][4][
            numero_snips
        ]
        position_snip = int(
            llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][5][
                numero_snips
            ]
        )
        rm = llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][6][
            numero_snips
        ]
        disease = llista_snps_transmembrana_taula_snip_regions[num_posicio_llista][7][
            numero_snips
        ]
...
for num_posicio_llista in range(len(llista_snps_transmembrana_taula_snip_regions)):
    for numero_regions in range(
        ...
        mycursor.execute(
            (
                "INSERT INTO tm_segments VALUES"
                + str((accesion_protein, region_protein_inici, region_protein_final))
            )
        )
        conn.commit()
